...
/* Uses GPIO Pin 22 example                    */
/* (c) Bruce Smith  - prog23a_22.s                */
...
@ addresses of messages and values
...
@ unmapGPIOmemory   
@ On completion need to remocve memory mapping
@ and close file

        MOV     R0, R5          @ memory to unmap
        MOV     R1, #pagesize   @ amount we mapped
        BL      munmap          @ unmap it
...
@ end program here
_exit:  POP {R8}                @ restore SP to entry level.
        POP {R9}
        MOV R7, #1
        SWI #0
