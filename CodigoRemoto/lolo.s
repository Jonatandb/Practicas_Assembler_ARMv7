...
cambiarColor:
        .fnstart
        push {lr}
        ldr r1, =rojo       // Cargar la dirección de color (supongamos que "rojo" es una secuencia de escape de color)
        mov r2, #6          // Longitud del mensaje de color, por ejemplo
        mov r0, #1          // Descriptor de archivo para salida estándar
        mov r7, #4          // Llamada al sistema de escritura
        svc #0              // Interrupción para ejecutar la llamada al sistema
        pop {lr}
        bx lr
        .fnend
