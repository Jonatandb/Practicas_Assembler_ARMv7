...
inicio_juego:
    ...
    ldr r1, =buffer_usuario                      // DirecciÃ³n de la palabra ingresada
    ldr r2, =palabra_oculta                      // DirecciÃ³n de la palabra oculta
    ldr r3, =resultado_buffer                    // DirecciÃ³n para almacenar el resultado
    bl comparar_letras                           // Llamar a la subrutina de comparaciÃ³n
    ...
    beq ganar                                    // Saltar a ganar si ganÃ³
...
comparar_letras:
    mov r4, #0                     // Ãndice para recorrer la palabra (0 a 4)
comparar_loop:
    ...
    // Si la letra no es igual, verificar si estÃ¡ en la palabra pero en otra posiciÃ³n
    // AquÃ­ puedes agregar la lÃ³gica para verificar si la letra estÃ¡ en la palabra (en otro lugar)
    mov r7, #'X'                   // Si la letra no estÃ¡ en la palabra, asignar "X" (gris)
...
siguiente_letra:
    add r4, r4, #1                 // Incrementar Ã­ndice
...
verificar_victoria:
    ...
    mov r4, #0                     // Ãndice
    ldr r5, =resultado_buffer       // DirecciÃ³n del buffer de resultado
    verificar_loop:
...
no_gano:
    mov r0, #1                      // Si no todas son "G", el juego continÃºa
    bx lr
